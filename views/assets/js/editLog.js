...
const request = {
    add : () => {
        ...
        form.append('updateLog', true);

        fetch('/updatingNewLog/'+parseInt(new URL(document.location).pathname.split("/").filter(id=>id).at(-1))+'?etr3545-3656-32dsdf-32dsd-fdhg-323-sads-23-sddfds-a='+new URL(document.location).searchParams.get('etr3545-3656-32dsdf-32dsd-fdhg-323-sads-23-sddfds-a'), {
            method: 'POST',
            body : form
        })
        .then(res => res.json())
